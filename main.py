...
# ★ 테스트 설정 ★
#   True  : 답을 적지 않아도 다음 문제로 넘어갈 수 있습니다. (지금은 시험용)
#   False : 답을 반드시 적어야 넘어갈 수 있습니다. (실제 수업 때는 False 로)
답_안써도_넘어가기 = True
...
안내문 = "💡 답을 모르겠으면 **'모르겠다'** 라고 적어 주세요. 문제를 넘기면 답은 **자동으로 저장**돼요."
if 답_안써도_넘어가기:
    안내문 += "  (시험용: 답을 비워 두고 넘어갈 수도 있어요)"
st.caption(안내문)
...
with 오른쪽칸:
    ...
    if st.button("다음 문제 ➡️", disabled=(번호 == 총문제수 - 1), use_container_width=True):
        if not 내답.strip() and not 답_안써도_넘어가기:
            st.warning("⚠️ 답을 적어야 다음 문제로 넘어갈 수 있어요. 모르겠으면 '모르겠다'라고 적어 주세요.")
        else:
            답_저장하기()                  # 넘어가면서 자동 저장 (빈칸이면 저장 안 함)
            st.session_state["문제번호"] += 1
            st.rerun()
...
if 번호 == 총문제수 - 1:
    ...
    if st.button("✅ 최종 제출하기", type="primary", use_container_width=True, disabled=종료):
        ...
        elif not 내답.strip() and not 답_안써도_넘어가기:
