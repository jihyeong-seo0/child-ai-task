...
# ---------------------------------------------------------
# 두 페이지의 AI 성격이 다릅니다.
#  · 인지과제      → 정답을 절대 알려주지 않고 생각만 도와줍니다.
#  · 창의적 글쓰기 → 정답이 없는 활동이므로 자유롭게 도와줍니다.

기본_성격 = "너는 따뜻하고 친절한 데이터 분석 선생님이야. 반드시 순수 한국어로만 답해. "
...
글쓰기_성격 = (
    "초등학생이 자기 생각을 자유롭게 펼치도록 도와줘. 정답이 없는 활동이니 "
    "여러 방향으로 생각을 넓혀 주고, 아이의 생각을 먼저 칭찬한 뒤 질문으로 더 깊게 이끌어 줘. "
    "초등학생이 이해할 수 있는 쉬운 말로 설명해."
)

# 예전 이름도 남겨 둡니다. (인지과제용)
SYSTEM_PROMPT = 기본_성격 + 정답금지_규칙
...
# ---------------------------------------------------------
def AI대화칸(페이지키, 이름, 학년, 문제_설명=None, 높이=380, 정답금지=True):
    ...
    # AI에게 줄 안내문 만들기
    #  · 정답금지=True  (인지과제)      → 정답을 알려주지 않는 성격
    #  · 정답금지=False (창의적 글쓰기) → 자유롭게 도와주는 성격
    시스템문 = 기본_성격 + (정답금지_규칙 if 정답금지 else 글쓰기_성격)
    if 문제_설명:
        시스템문 += f"\n\n[학생이 지금 하고 있는 활동]\n{문제_설명}"
        if 정답금지:
